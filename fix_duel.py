...
new_show_error = """  Future<void> _joinRoom(OpenDuelRoom room) async {
    ...
    final res = await SocialApiService.joinDuelRoom(room.id);
    if (!mounted) return;
    if (res.success && res.data != null) {
      sound.playTap();
      Navigator.push(context,
        MaterialPageRoute(builder: (_) => DuelLobbyScreen(roomId: room.id, roomUuid: room.uuid)));
    } else {
      sound.playWrong();
...
# Fix 4: Add sound to duel answer selection
old_answer = "  void _pickAnswer(int chosen) {"
idx = c.find(old_answer)
if idx > -1:
    end = c.find('\n  }', idx) + 4
    snippet = c[idx:end]
    new_snippet = snippet.replace(
        "  void _pickAnswer(int chosen) {",
        "  void _pickAnswer(int chosen) {\n    HapticFeedback.lightImpact();"
    )
    c = c.replace(snippet, new_snippet)
    print("4. Haptic on duel answer added!")
else:
    print("4. NOT FOUND")

# Fix 5: Add _OpenRoomCard widget at end of file before last closing
      ...
      child: Row(children: [
        ...
        Expanded(child: Column(
          ...
          children: [
            ...
            Text('Host: ${room.hostUsername} · ${room.participantCount}/${room.maxPlayers} players',
...
if old_empty_state in c:
    ...
    print("5. _OpenRoomCard widget added!")
else:
    print("5. NOT FOUND")

# Fix 6: Add sound effects to duel result (win/lose)
old_result = "class DuelResultScreen extends StatelessWidget {"
idx6 = c.find(old_result)
if idx6 > -1:
    print("6. DuelResultScreen found - adding sound")
    old_init = "  @override\n  Widget build(BuildContext context) {\n    final won = result.winnerId == currentUserId;"
    new_init = """  @override
  Widget build(BuildContext context) {
    final won = result.winnerId == currentUserId;
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      await sound.init();
      if (won) {
        await sound.playCoin();
      } else {
        await sound.playWrong();
      }
    });"""
    if old_init in c:
        c = c.replace(old_init, new_init)
        print("6. Win/lose sounds added!")
    else:
        print("6. Result init NOT FOUND")
else:
    print("6. DuelResultScreen NOT FOUND")
